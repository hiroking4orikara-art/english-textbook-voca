...
const AD_CONFIG = {
    // Platform: Android
    appId: 'ca-app-pub-6979818021800533~7331815510',
    interstitialId: 'ca-app-pub-6979818021800533/2267625861',
    bannerId: 'ca-app-pub-6979818021800533/9681335016', // Global bottom banner
...
const state = {
    ...
    quizCompletionCount: parseInt(localStorage.getItem('admob_interstitial_count') || '0', 10) // Counter for AdMob logic
};
...
function showResult() {
    ...
    // AdMob Logic: Show ad every 3rd quiz completion
    checkAndShowAd();
}
...
async function initAdMob() {
    try {
        ...
        preloadInterstitial(); // Start loading the first ad
        showBannerAd(); // Show global banner ad
    } catch (e) {
...
AdMob.addListener(BannerAdPluginEvents.FailedToLoad, (info) => {
    console.error("AdMob Event: Banner Failed To Load", info);
});

async function showBannerAd() {
    try {
        const adId = AD_CONFIG.isTesting ? TEST_BANNER_ID : AD_CONFIG.bannerId;
        const options = {
            adId: adId,
        ...
        console.log("AdMob: Banner Ad shown with ID:", adId);
    } catch (e) {
...
async function preloadInterstitial() {
    try {
        const adId = AD_CONFIG.isTesting ? TEST_INTERSTITIAL_ID : AD_CONFIG.interstitialId;
        const options = {
            adId: adId,
            isTesting: AD_CONFIG.isTesting,
            npa: true
...
async function checkAndShowAd() {
    state.quizCompletionCount++;
    localStorage.setItem('admob_interstitial_count', state.quizCompletionCount.toString());
    console.log(`Quiz Completion Count: ${state.quizCompletionCount}`);

    // Show ad if count % 3 == 0 (every 3rd time)
    if (state.quizCompletionCount % 3 === 0) {
...
async function showInterstitialAd() {
    if (isAdLoaded) {
        try {
            ...
            // If show fails, try to reload
            preloadInterstitial();
        }
    } else {
        console.log("AdMob: Ad not ready yet, skipping this time (will preload for next)");
        preloadInterstitial();
    }
}
...
async function renderPrintContent(type) {
    ...
    try {
        ...
        // Execute Print
        // alert("印刷を開始します..."); 
        await Printer.print({ content: fullHtml });
